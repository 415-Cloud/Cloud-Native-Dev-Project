...
### Patrick
-  I initially found the process of updating the frontend after deployment to be challenging. Changes I made and rebuilt were not appearing in the live application, which was confusing. I eventually realized that Kubernetes was not pulling the updated image because the tag remained the same. I learned that I needed to increment the container version number (e.g., v1.0 to v1.1) with each push to ensure Kubernetes recognized the change and deployed the new image.
- I struggled with connecting the services locally during our initial testing with Docker Compose. Orchestrating the communication between the frontend and backend services was complex, particularly because we were originally using multiple isolated databases for each service. Consolidating these connections to ensure the frontend could interact correctly with the backend required significant configuration and troubleshooting.
### Alex 
...
- Last issue, transitioning the leaderboard and user profile data from mock, in-memory data to persistent data stored in PostgreSQL. Initially, the leaderboard service returned mock data for development and UI testing. Once the data-consistency service was fully connected to PostgreSQL, the leaderboard service was updated to fetch and update real profile data through that service. This allowed user profiles and leaderboard data to persist across sessions and be dynamically updated in the application.
